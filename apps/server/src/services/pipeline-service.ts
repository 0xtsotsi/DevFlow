...
function getPipelineConfigPath(projectPath: string): string {
  ...
  return path.join(automakerDir, 'pipeline.json');
}
...
export class PipelineService {
  ...
  getNextStatus(
    ...
    if (currentStatus === 'in_progress') {
      return `pipeline_${sortedSteps[0].id}` as FeatureStatusWithPipeline;
    }
    ...
    if (currentStatus.startsWith('pipeline_')) {
      ...
      if (currentIndex < sortedSteps.length - 1) {
        // Go to next step
        return `pipeline_${sortedSteps[currentIndex + 1].id}` as FeatureStatusWithPipeline;
      }
